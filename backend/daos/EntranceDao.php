<?php
 if (!defined("ENTRANCE_DAO_INC")) {
	define("ENTRANCE_DAO_INC",1);
	
	class EntranceDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getState() {
			return array(
                            ...
                            'filter_season' => Generals::getState('filter.season'), 'filter_service' => Generals::getState('filter.service'),
        ...
        function getExistedCode($code, $void, $parent = null) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_entrance AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
        ...
        function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_entrance_lang WHERE entrance = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_entrance AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_entrance AS a ';
			$query.= ' INNER JOIN tbl_entrance_lang AS b ON a.id = b.entrance WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array(Generals::getSession('langcode')));
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, b.name, b.alias, b.introtext FROM tbl_entrance AS a ';
			$query.= ' LEFT JOIN tbl_entrance_lang AS b ON a.id = b.entrance';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
        ...
        function getSuppliers(){
            ...
            $query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
            $query.= ' WHERE a.published = 1 AND b.language = ? AND a.services LIKE "%,entrance,%" ORDER BY b.name ';
        ...
        function getServices($id = null){
            ...
            $query.= ' WHERE a.published = 1 AND b.language = ? AND c.language = ? AND s.services LIKE "%,entrance,%" ';
            if ($id) $query.= ' AND a.id = '.(int)$id;
            $query.= ' AND d.language = ? AND a.genre = "entrance" ORDER BY d.name, b.name ';
        ...
        function _buildQuery(){
			...
			$query = ' SELECT a.*, b.name, b.alias, b.introtext, v.name AS sv_name, ';
            $query.= ' p.name AS su_name, c.name AS lo_name, s.name AS se_name ';
            $query.= ' FROM tbl_entrance AS a ';
			$query.= ' LEFT JOIN tbl_entrance_lang AS b ON a.id = b.entrance';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_entrance SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_entrance SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_entrance', " id = ? ", array($oid));
			$this->delete_db('tbl_entrance_lang', " entrance = ? ", array($oid));
		}
	}// end class		
 }
?>