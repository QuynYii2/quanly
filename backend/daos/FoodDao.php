<?php
 if (!defined("FOOD_DAO_INC")) {
	define("FOOD_DAO_INC",1);
	
	class FoodDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void, $parent = null) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_food AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_food_lang WHERE food = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_food AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_food AS a ';
			$query.= ' INNER JOIN tbl_food_lang AS b ON a.id = b.food WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array(Generals::getSession('langcode')));
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, b.name, b.alias, b.introtext FROM tbl_food AS a ';
			$query.= ' LEFT JOIN tbl_food_lang AS b ON a.id = b.food';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
        ...
        function getServices($id = null){
            ...
            $query.= ' WHERE a.published = 1 AND b.language = ? AND c.language = ? AND s.services LIKE "%,food,%" ';
            if ($id) $query.= ' AND a.id = '.(int)$id;
            $query.= ' AND d.language = ? AND a.genre = "food" ORDER BY d.name, b.name ';
        ...
        function _buildQuery(){
            ...
            $query.= ' p.name AS su_name, c.name AS lo_name, s.name AS se_name FROM tbl_food AS a ';
			$query.= ' LEFT JOIN tbl_food_lang AS b ON a.id = b.food';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_food SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_food SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_food', " id = ? ", array($oid));
			$this->delete_db('tbl_food_lang', " food = ? ", array($oid));
		}
	}// end class		
 }
?>