<?php
 if (!defined("PAYMENT_DAO_INC")) {
	define("PAYMENT_DAO_INC",1);
	
	class PaymentDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_payment AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
            $this->prepare($query);
        ...
        function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_payment_lang WHERE payment = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_payment AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_payment AS a ';
			$query.= ' INNER JOIN tbl_payment_lang AS b ON a.id = b.payment WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_payment AS a ';
			$query.= ' LEFT JOIN tbl_payment_lang AS fl ON a.id = fl.payment';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_payment SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_payment SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_payment', " id = ? ", array($oid));
			$this->delete_db('tbl_payment_lang', " payment = ? ", array($oid));
		}
	}// end class		
 }
?>