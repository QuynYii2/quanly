<?php
 if (!defined("SPOKEN_DAO_INC")) {
	define("SPOKEN_DAO_INC",1);
	
	class SpokenDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_spoken AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
            $this->prepare($query);
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_spoken_lang WHERE spoken = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_spoken AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_spoken AS a ';
			$query.= ' INNER JOIN tbl_spoken_lang AS b ON a.id = b.spoken WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_spoken AS a ';
			$query.= ' LEFT JOIN tbl_spoken_lang AS fl ON a.id = fl.spoken';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_spoken SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_spoken SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_spoken', " id = ? ", array($oid));
			$this->delete_db('tbl_spoken_lang', " spoken = ? ", array($oid));
		}
	}// end class		
 }
?>