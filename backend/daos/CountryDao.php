<?php
 if (!defined("COUNTRY_DAO_INC")) {
	define("COUNTRY_DAO_INC",1);
	
	class CountryDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_country AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
            $this->prepare($query);
        ...
        function getData($void = null) {
			...
			$query 	= 'SELECT a.* FROM tbl_country AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, a.name AS text FROM tbl_country AS a ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.* FROM tbl_country AS a '.$where;
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_country SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_country SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getData($oid);
			$this->delete_db('tbl_country', " id = ? ", array($oid));
