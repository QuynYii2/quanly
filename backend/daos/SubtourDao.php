<?php
 if (!defined("SUBTOUR_DAO_INC")) {
	define("SUBTOUR_DAO_INC",1);
	
	class SubtourDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void, $parent = null) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_subtour AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
        ...
        function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_subtour_lang WHERE subtour = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_subtour AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_subtour AS a ';
			$query.= ' INNER JOIN tbl_subtour_lang AS b ON a.id = b.subtour WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array(Generals::getSession('langcode')));
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, b.name, b.alias, b.introtext FROM tbl_subtour AS a ';
			$query.= ' LEFT JOIN tbl_subtour_lang AS b ON a.id = b.subtour';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
        ...
        function getServices($id = null){
            ...
            $query.= ' WHERE a.published = 1 AND b.language = ? AND c.language = ? AND s.services LIKE "%,subtour,%" ';
            if ($id) $query.= ' AND a.id = '.(int)$id;
            $query.= ' AND d.language = ? AND a.genre = "subtour" ORDER BY d.name, b.name ';
        ...
        function _buildQuery(){
            ...
            $query.= ' p.name AS su_name, s.name AS se_name FROM tbl_subtour AS a ';
			$query.= ' LEFT JOIN tbl_subtour_lang AS b ON a.id = b.subtour';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_subtour SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_subtour SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_subtour', " id = ? ", array($oid));
			$this->delete_db('tbl_subtour_lang', " subtour = ? ", array($oid));
		}
	}// end class		
 }
?>