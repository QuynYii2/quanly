<?php
 if (!defined("EXPTRANS_DAO_INC")) {
	define("EXPTRANS_DAO_INC",1);
	
	class ExpTransDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getProfileService($qid, $genre){
            ...
            $query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
            $query.= ' WHERE c.quotation = ? AND a.genre IN ("vehicle", "boat") AND c.service IN ("vehicle", "boat") AND l.code = ? ';
            $this->prepare($query);
            $result = $this->exec(array($qid, Generals::getSession('langcode')));
