<?php
 if (!defined("BOAT_DAO_INC")) {
	define("BOAT_DAO_INC",1);
	
	class BoatDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void, $parent = null) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_boat AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
        ...
        function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_boat_lang WHERE boat = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_boat AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_boat AS a ';
			$query.= ' INNER JOIN tbl_boat_lang AS b ON a.id = b.boat WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array(Generals::getSession('langcode')));
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, b.name, b.alias, b.introtext FROM tbl_boat AS a ';
			$query.= ' LEFT JOIN tbl_boat_lang AS b ON a.id = b.boat';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
        ...
        function getServices($id = null){
            ...
            $query.= ' WHERE a.published = 1 AND b.language = ? AND c.language = ? AND s.services LIKE "%,boat,%" ';
            if ($id) $query.= ' AND a.id = '.(int)$id;
            $query.= ' AND d.language = ? AND a.genre = "boat" ORDER BY d.name, b.name ';
        ...
        function _buildQuery(){
            ...
            $query.= ' p.name AS su_name, c.name AS lo_name, s.name AS se_name FROM tbl_boat AS a ';
			$query.= ' LEFT JOIN tbl_boat_lang AS b ON a.id = b.boat';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_boat SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_boat SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_boat', " id = ? ", array($oid));
			$this->delete_db('tbl_boat_lang', " boat = ? ", array($oid));
		}
	}// end class		
 }
?>