<?php
 if (!defined("SEASON_DAO_INC")) {
	define("SEASON_DAO_INC",1);
	
	class SeasonDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_season AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
            $this->prepare($query);
        ...
        function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_season_lang WHERE season = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_season AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_season AS a ';
			$query.= ' INNER JOIN tbl_season_lang AS b ON a.id = b.season WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_season AS a ';
			$query.= ' LEFT JOIN tbl_season_lang AS fl ON a.id = fl.season';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_season SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_season SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_season', " id = ? ", array($oid));
			$this->delete_db('tbl_season_lang', " season = ? ", array($oid));
		}
	}// end class		
 }
?>