 ...
 if (!defined("SUPPLIER_DAO_INC")) {
	...
	class SupplierDao extends DbConnect /*DbMySQLConnect*/  {
        ...
        function getExistedCode($code, $void) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_supplier AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
            $this->prepare($query);
		...
		function _buildQuery(){
            ...
            if (strlen($service)):
                $where[] = ' a.services LIKE ? ';
                array_push($params, '%,'.$service.',%');
            endif;

            $where = ( count( $where ) ? ' WHERE '. implode( ' AND ', $where ) : '' );
			$query = ' SELECT a.*, b.name, b.alias, b.introtext, c.name AS lo_name FROM tbl_supplier AS a ';
			$query.= ' LEFT JOIN tbl_supplier_lang AS b ON a.id = b.supplier';
		...
		public function delete($oid) {
            ...
            #=======================================================================
            $this->prepare(" SELECT * FROM tbl_supplier WHERE id = ".(int)$oid);
            ...
            $introtext = 'Delete Table: tbl_supplier<br/>';
            $introtext.= 'Where id = '.$oid.'<br/>';
            $introtext.= 'Old Datas:';
            $introtext.= '<p>'.implode('<br/>', $old_datas).'</p>';

            $logdata = array();
            $logdata['title']       = 'Delete Table: tbl_supplier';
