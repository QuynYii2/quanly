 ...
 if (!defined("SERVICE_DAO_INC")) {
	...
	class ServiceDao extends DbConnect /*DbMySQLConnect*/  {
		...
		function __construct() {
            ...
            $this->_genre = Generals::getVar('genre', '');
			$this->setState();
			$this->setId($cid[0]);
			parent::__construct();
		}
		...
		function setState() {
			if (!is_null(Generals::getVar('filter_search'))) 	Generals::setState($this->_genre.'.filter.search', strtolower(trim(Generals::getVar('filter_search'))));
			if (!is_null(Generals::getVar('filter_published'))) Generals::setState($this->_genre.'.filter.published', Generals::getVar('filter_published'));
            if (!is_null(Generals::getVar('filter_supplier')))  Generals::setState($this->_genre.'.filter.supplier', Generals::getVar('filter_supplier'));
            if (!is_null(Generals::getVar('filter_location')))  Generals::setState($this->_genre.'.filter.location', Generals::getVar('filter_location'));
			if (!is_null(Generals::getVar('page'))) 			Generals::setState($this->_genre.'.page', Generals::getVar('page', 1));
			if (!is_null(Generals::getVar('orderby_order'))) 	Generals::setState($this->_genre.'.orderby.order', Generals::getVar('orderby_order', 'ASC'));
			if (!is_null(Generals::getVar('orderby_field'))) 	Generals::setState($this->_genre.'.orderby.field', Generals::getVar('orderby_field', 'a.id'));
		}
		
		function getState() {
			return array(
							'filter_search' => Generals::getState($this->_genre.'.filter.search'), 'filter_published' => Generals::getState($this->_genre.'.filter.published'),
							'orderby_order' => Generals::getState($this->_genre.'.orderby.order'), 'orderby_field' => Generals::getState($this->_genre.'.orderby.field'),
                            'filter_supplier' => Generals::getState($this->_genre.'.filter.supplier'), 'filter_location' => Generals::getState($this->_genre.'.filter.location')
						);
		}

        function getExistedCode($code, $void) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_service AS a WHERE TRIM(LOWER(a.genre)) = ? AND TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
            $this->prepare($query);
            $result = $this->exec(array(trim(strtolower($this->_genre)), trim(strtolower($code)), (int)$void));
        ...
        function getSuppliers(){
            ...
            $query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
            $query.= ' WHERE a.published = 1 AND b.language = ? AND a.services LIKE "%,'.$this->_genre.',%" ORDER BY b.name ';
        ...
        function _buildQuery(){
			$search 	= Generals::getState($this->_genre.'.filter.search');
			$search 	= JString::strtolower($search);
			$published 	= Generals::getState($this->_genre.'.filter.published');
            $supplier 	= Generals::getState($this->_genre.'.filter.supplier');
            $location 	= Generals::getState($this->_genre.'.filter.location');
            ...
            if (strlen($location)):
                $where[] = ' a.location = ? ';
                array_push($params, (int)$location);
            endif;
			...
			Generals::setState($this->_genre.'.params', $params);

			return $query;
		}
		
		function getDataList() {
			$field = Generals::getState($this->_genre.'.orderby.field', 'a.id') ? Generals::getState($this->_genre.'.orderby.field', 'a.id') : 'a.id';
			$order = Generals::getState($this->_genre.'.orderby.order', 'DESC') ? Generals::getState($this->_genre.'.orderby.order', 'DESC') : 'DESC';
			$query = $this->_buildQuery();
			$query.= ' ORDER BY '.$field.' '.$order;
			$this->prepare($query);
			$result = $this->exec(Generals::getState($this->_genre.'.params'));
			$this->total = sizeof($result);

			while (true && $this->total):
				$offset = (int)(Generals::getState($this->_genre.'.page', 1)-1)*LIMIT_RECORD;
				if ($offset >= $this->total) {
					$offset	= (int)$offset - (int)LIMIT_RECORD;
					Generals::setState('page', Generals::getState($this->_genre.'.page', 1)-1);
				} elseif ($offset < 0) {
					$offset = 0;
					break;
				} else {
			...
			if (is_array($result)):
				...
				$page	= Generals::getState($this->_genre.'.page', 1)-1 >= 0 ? Generals::getState($this->_genre.'.page', 1)-1 : 0;
			else:
				$page = 0;
				$result[0] = $result;
			endif;
