<?php
 if (!defined("STATE_DAO_INC")) {
	define("STATE_DAO_INC",1);
	
	class StateDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getCountries() {
			$query = 'SELECT a.* FROM tbl_country AS a WHERE a.published = 1 ORDER BY name ASC ';
			$this->prepare($query);
		...
		function getStates($city = null) {
			$query = ' SELECT a.* FROM tbl_state AS a WHERE a.published = 1 ';
			if ($city) $query.= ' AND a.city = '.(int)$city;
			$query.= ' ORDER BY name ASC ';
			$this->prepare($query);
			$result = $this->exec(array());
		...
		function getData($void = null) {
			...
			$query 	= 'SELECT a.* FROM tbl_state AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, a.name AS text FROM tbl_state AS a ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array());
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.*, b.name AS country_name, c.name AS city_name FROM tbl_state AS a ';
			$query.= ' INNER JOIN tbl_country AS b ON a.country = b.id ';
			$query.= ' INNER JOIN tbl_city AS c ON a.city = c.id '.$where;
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_state SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_state SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getData($oid);
			$this->delete_db('tbl_state', " id = ? ", array($oid));
