 ...
 if (!defined("HOTEL_DAO_INC")) {
	...
	class HotelDao extends DbConnect /*DbMySQLConnect*/  {
        ...
        function getExistedCode($code, $void, $parent = null) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_hotel AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, b.name, b.alias, b.introtext FROM tbl_hotel AS a ';
			$query.= ' LEFT JOIN tbl_hotel_lang AS b ON a.id = b.hotel';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
        ...
        function getServices($id = null){
            ...
            $query.= ' WHERE a.published = 1 AND b.language = ? AND c.language = ? AND s.services LIKE "%,hotel,%" ';
            if ($id) $query.= ' AND a.id = '.(int)$id;
            $query.= ' AND d.language = ? AND a.genre = "hotel" ORDER BY d.name, b.name ';
		...
		function getDataList($getall = false) {
			...
			if (empty($this->total)) return array();
