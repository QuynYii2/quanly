 ...
 if (!defined("USER_DAO_INC")) {
	...
	class UserDao extends DbConnect /*DbMySQLConnect*/  {
		...
		function getLoginData($username, $password) {
			...
			$query.= ' WHERE TRIM(LOWER(a.username)) = ? AND a.password = ? AND a.published = 1 AND b.language = ? ';
			$this->prepare($query);
			
			$result = $this->exec(array(trim(strtolower($username)), md5($password), Generals::getSession('langcode')));
