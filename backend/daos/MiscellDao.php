<?php
 if (!defined("MISCELL_DAO_INC")) {
	define("MISCELL_DAO_INC",1);
	
	class MiscellDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void, $parent = null) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_miscell AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
        ...
        function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_miscell_lang WHERE miscell = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_miscell AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_miscell AS a ';
			$query.= ' INNER JOIN tbl_miscell_lang AS b ON a.id = b.miscell WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array(Generals::getSession('langcode')));
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, b.name, b.alias, b.introtext FROM tbl_miscell AS a ';
			$query.= ' LEFT JOIN tbl_miscell_lang AS b ON a.id = b.miscell';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
        ...
        function getServices($id = null){
            ...
            $query.= ' WHERE a.published = 1 AND b.language = ? AND c.language = ? AND s.services LIKE "%,miscell,%" ';
            if ($id) $query.= ' AND a.id = '.(int)$id;
            $query.= ' AND d.language = ? AND a.genre = "miscell" ORDER BY d.name, b.name ';
        ...
        function _buildQuery(){
			...
			$query = ' SELECT a.*, b.name, b.alias, b.introtext, v.name AS sv_name, c.name AS lo_name, ';
            $query.= ' p.name AS su_name, s.name AS se_name FROM tbl_miscell AS a ';
			$query.= ' LEFT JOIN tbl_miscell_lang AS b ON a.id = b.miscell';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_miscell SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_miscell SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_miscell', " id = ? ", array($oid));
			$this->delete_db('tbl_miscell_lang', " miscell = ? ", array($oid));
		}
	}// end class		
 }
?>