<?php
 if (!defined("PERSON_DAO_INC")) {
	define("PERSON_DAO_INC",1);
	
	class PersonDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
        ...
        function getExistedCode($code, $void) {
            $query = 'SELECT COUNT(a.id) AS count FROM tbl_person AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
            $this->prepare($query);
        ...
        function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_person_lang WHERE person = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_person AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_person AS a ';
			$query.= ' INNER JOIN tbl_person_lang AS b ON a.id = b.person WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_person AS a ';
			$query.= ' LEFT JOIN tbl_person_lang AS fl ON a.id = fl.person';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_person SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_person SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_person', " id = ? ", array($oid));
			$this->delete_db('tbl_person_lang', " person = ? ", array($oid));
		}
	}// end class		
 }
?>