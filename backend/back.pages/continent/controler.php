...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=continent&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=continent&view=form');
		return false;
	}
	...
	function save($task) {
		...
		$vform['id'] = (int)$cid[0];
        $vform["hotelrate"] = $vform["hotelrate"] ? $vform["hotelrate"] : 1;
        $vform["otherrate"] = $vform["otherrate"] ? $vform["otherrate"] : 1;
		...
		#=========================================================================================
		if ($vform["remsocial"]):
			$DiskPath = IMG_PATH.DS."continent";
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."continent")) {
				$link= 'index.php?option=continent&view=form&cid[]='. $vform['id'];
		...
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_continent');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_continent', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_continent', $data);
			endif;
	        ...
	        if ($task == "save"):
	        	$link = 'index.php?option=continent&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=continent&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_continent_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_continent_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
