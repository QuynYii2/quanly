...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=food&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=food&view=form');
		return false;
	}
    ...
    function save($task) {
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."food")) {
				$link= 'index.php?option=food&view=form&cid[]='. $vform['id'];
            ...
            JFile::copy(IMG_PATH.DS."food".DS.Generals::getVar('hiddensocial'), IMG_PATH.DS."food".DS.$vform['social']);
        }
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_food');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_food', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_food', $data);
			endif;
            ...
            if ($task == "save" || $task == "copy"):
	        	$link = 'index.php?option=food&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=food&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_food_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_food_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
