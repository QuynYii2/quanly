...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=boat&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=boat&view=form');
		return false;
	}
    ...
    function save($task) {
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."boat")) {
				$link= 'index.php?option=boat&view=form&cid[]='. $vform['id'];
            ...
            JFile::copy(IMG_PATH.DS."boat".DS.Generals::getVar('hiddensocial'), IMG_PATH.DS."boat".DS.$vform['social']);
        }
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_boat');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_boat', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_boat', $data);
			endif;
            ...
            if ($task == "save" || $task == "copy"):
	        	$link = 'index.php?option=boat&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=boat&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_boat_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_boat_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
