...
class controler {
    ...
    function getDetailService() {
        ...
        $return.= ' <td class="text-center prices" width="16.6667%" style="padding: 5px 3px;">'.number_format($result['c_cost']*$convert, 2).'</td>';
        $return.= ' <td class="text-center prices" width="16.6667%" style="padding: 5px 3px;">'.number_format($result['cost']*$convert, 2).'<input type="hidden" id="control-'.$day.'-'.$num.'-7" name="vdata[price]['.$day.']['.$num.']" field="price" class="form-control number price" value="'.number_format($result['cost']*$convert, 2).'"><input type="hidden" name="vdata[update]['.$day.']['.$num.']" class="update" value="1"/></td>';
        ...
        $return.= ' <td class="text-center perpax" width="16.6667%" style="padding: 5px 3px;">'.$c_price.'</td>';
        $return.= ' <td class="text-center perpax" width="16.6667%" style="padding: 5px 3px;">'.$price.'</td>';
        $return.= ' <td class="text-center single" width="16.6667%" style="padding: 5px 3px;">'.(!empty($c_single) ? $c_single : '').'</td>';
        $return.= ' <td class="text-center single" width="16.6667%" style="padding: 5px 3px;">'.(!empty($single) ? $single : '').'</td>';
        $return.= '</tr>\n';
    ...
    function getPriceFormat($genre, $subid, $convert = 1, $price = 0, $paxgroup = 0, $saleof = 0, $issing = 0) {
        $convert= $convert ? $convert : 1;
        $result = $this->_db->getDetailService($genre, $subid);
        $person = $this->_db->getPerson($paxgroup);
        $price  = $price ? str_replace(',', '', $price) : 0;
