...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=currencies&view=list');
	}
	...
	function create() {
        ...
        header('Location: index.php?option=currencies&view=form');
		return false;
	}
	...
	function save($task) {
        ...
        if ($vform["remimage"]):
			$DiskPath = IMG_PATH.DS."currencies";
		...
		if($file_image['name'] != '')  {
			if (!Generals::addFile('image', null, IMG_PATH.DS."currencies")) {
				$link= 'index.php?option=currencies&view=form&cid[]='. $vform['id'];
				Generals::redirect($link); return;
			} else {
				$vform['image'] = Generals::getVar('image');
				$vform['thumb'] = str_replace('image', 'thumb', $vform['image']);
				JFolder::create(IMG_PATH.DS."currencies".DS.'resize');
				ImageClass::createImage(IMG_PATH.DS."currencies".DS.$vform['image'], IMG_PATH.DS."currencies".DS.'resize'.DS.$vform['thumb'], 100, 100);
			}
		}
		$data = $this->_db->getDataMapTable($vform, 'tbl_currencies');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_currencies', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_currencies', $data);
			endif;
	        ...
	        if ($task == "save"):
	        	$link = 'index.php?option=currencies&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=currencies&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
