...
class controler {
	...
	function save($task) {
        ...
        **/
        ...
        if ($task == 'export'):
            Generals::getError();
            $this->_db->prepare('UPDATE tbl_invoice SET export = export + 1 WHERE id  = ?');
            $this->_db->exec(array((int)$vform['id']));
    ...
    function export2PDF($data) {
        ...
        $totals     = 0;
        $paid       = 0;
        foreach ($introtext['intro']['intro'] as $key => $val):
            $paid+= $items['paid'];
            $html.= '   <tr>';
            $html.= '       <td valign="top" style="border-top: #000000 1px solid;border-left: #000000 1px solid;padding: 3px 5px;">'.$val.'</td>';
            $html.= '       <td valign="top" style="border-top: #000000 1px solid;border-left: #000000 1px solid;border-right: #000000 1px solid;padding: 3px 5px;">'.number_format($introtext['intro']['amount'][$key], 2, '.', '').'</td>';
            $html.= '   </tr>';
            $totals+= $items['total'];
        endforeach;
        ...
        **/
        $html.= '   <tr>';
        $html.= '       <td valign="top" align="right"><strong>'.Generals::getTitle('INV_RECEIVABLE').':</strong> USD </td>';
        $html.= '       <td valign="top">'.number_format($paid, 2, '.', '').'</td>';
        $html.= '   </tr>';
        $html.= '   <tr>';
        $html.= '       <td valign="top" align="right"><strong>'.Generals::getTitle('INV_BALANCE').':</strong> USD </td>';
        $html.= '       <td valign="top">'.str_replace('-', '+', number_format($totals - $paid, 2, '.', '')).'</td>';
        $html.= '   </tr>';
        $html.= '   <tr>';
        $html.= '       <td valign="top" colspan="2" align="right">&nbsp;&nbsp;</td>';
        $html.= '   </tr>';
        $html.= '   <tr>';
        $html.= '       <td valign="top" colspan="2">'.$data['awords'].'</td>';
        $html.= '   </tr>';
        $html.= '</table>';
