...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=payment&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=payment&view=form');
		return false;
	}
	...
	function save($task) {
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."payment")) {
				$link= 'index.php?option=payment&view=form&cid[]='. $vform['id'];
		...
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_payment');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_payment', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_payment', $data);
			endif;
	        ...
	        if ($task == "save"):
	        	$link = 'index.php?option=payment&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=payment&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_payment_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_payment_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
