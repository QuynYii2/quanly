...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=exptrans&view=list');
	}
	...
	function create() {
        ...
        header('Location: index.php?option=exptrans&view=form');
		return false;
	}
	...
	function save($task) {
        ...
        $vform["profile"]   = !empty($vform["profile"]) ? ','.implode(',', $vform["profile"]).',' : null;
		...
		if($file_image['name'] != '')  {
			if (!Generals::addFile('image', null, IMG_PATH.DS."exptrans")) {
				$link= 'index.php?option=exptrans&view=form&cid[]='. $vform['id'];
				Generals::redirect($link); return;
			} else {
				$vform['image'] = Generals::getVar('image');
				$vform['thumb'] = str_replace('image', 'thumb', $vform['image']);
				JFolder::create(IMG_PATH.DS."exptrans".DS.'resize');
				ImageClass::createImage(IMG_PATH.DS."exptrans".DS.$vform['image'], IMG_PATH.DS."exptrans".DS.'resize'.DS.$vform['thumb'], 100, 100);
			}
		}
		$data = $this->_db->getDataMapTable($vform, 'tbl_export');
        $data['trans'] = $this->getJouneyService($data);
        $data['trans'] = json_encode($data['trans']);
		...
		try {
	        ...
	        if ($task == "save"):
	        	$link = 'index.php?option=exptrans&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=exptrans&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
    ...
    function getProfiles(){
        $profiles   = $this->_db->getProfileService(Generals::getVar('qid'), array('vehicle', 'boat'));
        $pids       = Generals::getVar('pids', array());
        $pids       = explode(',', $pids);
        $return     = '<option value="">'.Generals::getTitle('EXP_TRANS_SELECT').'</option>';
        if (is_array($profiles)) foreach ($profiles as $items):
            $return.= '<option value="'.$items['id'].'" '.(in_array($items['id'], $pids) ? 'selected="selected"': '').'>'.$items['code'].' - '.$items['name'].'</option>';
        endforeach;
        echo $return;
        exit();
    }
    ...
    function getQuotation(){
        ...
        $query.= ' WHERE a.genre = "vehicle" AND js.service = "vehicle" AND js.quotation = ? ';
    ...
    function getJouneyService($data) {
        $query = ' SELECT b.name, c.capacity ';
        $query.= ' FROM tbl_journey_service AS a INNER JOIN tbl_vehicle_lang AS b ON a.detail = b.vehicle ';
        $query.= ' INNER JOIN tbl_vehicle AS c ON c.id = b.vehicle ';
        $query.= ' INNER JOIN tbl_quotation AS q ON q.id = a.quotation ';
        $query.= ' WHERE a.service = "vehicle" AND a.quotation = ? AND b.language = ? ';
        if ($data['profile']) $query.= ' AND a.profile IN (0'.$data['profile'].'0) ';
        $query.= ' ORDER BY a.id ASC ';

        $this->_db->prepare($query);
        $cars = $this->_db->exec(array($data['quotation'], Generals::getSession('langcode')));

        $query = ' SELECT b.name, c.capacity ';
        $query.= ' FROM tbl_journey_service AS a INNER JOIN tbl_boat_lang AS b ON a.detail = b.boat ';
        $query.= ' INNER JOIN tbl_boat AS c ON c.id = b.boat ';
        $query.= ' INNER JOIN tbl_quotation AS q ON q.id = a.quotation ';
        $query.= ' WHERE a.service = "boat" AND a.quotation = ? AND b.language = ? ';
        if ($data['profile']) $query.= ' AND a.profile IN (0'.$data['profile'].'0) ';
        $query.= ' ORDER BY a.id ASC ';

        $this->_db->prepare($query);
        $boat = $this->_db->exec(array($data['quotation'], Generals::getSession('langcode')));

        $return = array();
        $next = 0;

        foreach ($cars as $items):
            $return[$next]['name']   = $items['name'];
            $return[$next]['seat']   = $items['capacity'];
            $next++;
        endforeach;

        foreach ($boat as $items):
            $return[$next]['name']   = $items['name'];
            $return[$next]['seat']   = $items['capacity'];
            $next++;
        endforeach;
    ...
    function export2PDF($data) {
        ...
        $html.= '   </tr>';
        $html.= '</table>';
        $html.= '<h1 style="text-align: center;">'.Generals::getTitle('EXP_GENRE_3').'</h1>';
        $html.= '<table style="width: 100%;border-collapse: 5px;border-spacing: 5px;">';
        $html.= '   <tr>';
        ...
        $html.= '   <tr>';
        $html.= '       <td valign="top" style="border-top: #000000 1px solid;border-left: #000000 1px solid;padding: 3px 5px;"><strong>'.Generals::getTitle('EXP_CAR_CATE').'</strong> </td>';
        $html.= '       <td valign="top" style="border-top: #000000 1px solid;border-left: #000000 1px solid;border-right: #000000 1px solid;padding: 3px 5px;text-align: center;"><strong>'.Generals::getTitle('EXP_CAR_SEAT').'</strong> </td>';
        $html.= '   </tr>';

        $trans = $data['trans'] ? json_decode($data['trans']) : array();
        foreach ($trans as $items):
            ...
            $html.= '       <td valign="top" style="border-top: #000000 1px solid;border-left: #000000 1px solid;border-right: #000000 1px solid;padding: 3px 5px;text-align: center;">'.$items->seat.'</td>';
            $html.= '   </tr>';
        endforeach;
        $html.= '   <tr>';
        $html.= '       <td valign="top" colspan="2" align="right" style="border-top: #000000 1px solid;"></td>';
        $html.= '   </tr>';
        $html.= '</table>';
        ...
        $mpdf = new mPDF ();
        $mpdf->WriteHTML ( $html );
        $mpdf->Output ( 'trans_voucher_'.$data['voucher'].'.pdf', 'D' );
