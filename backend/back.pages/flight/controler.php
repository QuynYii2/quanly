...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=flight&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=flight&view=form');
		return false;
	}
    ...
    function save($task) {
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."flight")) {
				$link= 'index.php?option=flight&view=form&cid[]='. $vform['id'];
            ...
            JFile::copy(IMG_PATH.DS."flight".DS.Generals::getVar('hiddensocial'), IMG_PATH.DS."flight".DS.$vform['social']);
        }
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_flight');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_flight', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_flight', $data);
			endif;
            ...
            if ($task == "save" || $task == "copy"):
	        	$link = 'index.php?option=flight&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=flight&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_flight_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_flight_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
