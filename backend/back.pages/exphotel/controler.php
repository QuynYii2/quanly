...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=exphotel&view=list');
	}
	...
	function create() {
        ...
        header('Location: index.php?option=exphotel&view=form');
		return false;
	}
	...
	function save($task) {
		...
		if($file_image['name'] != '')  {
			if (!Generals::addFile('image', null, IMG_PATH.DS."exphotel")) {
				$link= 'index.php?option=exphotel&view=form&cid[]='. $vform['id'];
				Generals::redirect($link); return;
			} else {
				$vform['image'] = Generals::getVar('image');
				$vform['thumb'] = str_replace('image', 'thumb', $vform['image']);
				JFolder::create(IMG_PATH.DS."exphotel".DS.'resize');
				ImageClass::createImage(IMG_PATH.DS."exphotel".DS.$vform['image'], IMG_PATH.DS."exphotel".DS.'resize'.DS.$vform['thumb'], 100, 100);
			}
		}
		...
		try {
	        ...
	        if ($task == "save"):
	        	$link = 'index.php?option=exphotel&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=exphotel&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
    ...
    function getQuotation(){
        ...
        $query.= ' WHERE a.genre = "hotel" AND js.service = "hotel" AND js.quotation = ? ';
        ...
        $result = $result[0];
        $result['e_date'] = $result['s_date'] ? date('d-m-Y', strtotime('+'.$result['numday'].' day', strtotime($result['s_date']))) : '';
    ...
    function getProfiles(){
        $profiles   = $this->_db->getProfileService(Generals::getVar('qid'), 'hotel');
        $pids       = Generals::getVar('pids', array());
        $pids       = explode(',', $pids);
        $return     = '<option value="">'.Generals::getTitle('EXP_HOTEL_SELECT').'</option>';
    ...
    function getProfile(){
        $query = ' SELECT a.*, b.name FROM tbl_service AS a ';
        $query.= ' LEFT JOIN tbl_service_lang AS b ON a.id = b.service';
        $query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
        $query.= ' WHERE a.genre = "hotel" AND b.language = ? AND a.id = ? ';
        $pids  = Generals::getVar('pid');
        $pids  = !is_array($pids) ? array($pids) : $pids;

        $this->_db->prepare($query);
        $result = $this->_db->exec(array(Generals::getSession('langcode'), $pids[0]));
        $result = $result[0];
    ...
    function getJouneyService($data) {
        ...
        $query.= ' WHERE a.service = "hotel" AND a.quotation = ? AND b.language = ? ';
    ...
    function export2PDF($data) {
        ...
        $html.= '   </tr>';
        $html.= '</table>';
        $html.= '<h1 style="text-align: center;">'.Generals::getTitle('EXP_GENRE_2').'</h1>';
        $html.= '<table style="width: 100%;border-collapse: 5px;border-spacing: 5px;">';
        $html.= '   <tr>';
        ...
        $mpdf = new mPDF ();
        $mpdf->WriteHTML ( $html );
        $mpdf->Output ( 'hotel_voucher_'.$data['voucher'].'.pdf', 'D' );
