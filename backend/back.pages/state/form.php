<?php
	...
	if ($cid[0]) $smarty->assign("TitlePage", Generals::getTitle('MENU_STATE') ." <span>[".Generals::getTitle('MENU_SUB_EDIT')."]</span>");
	else $smarty->assign("TitlePage", Generals::getTitle('MENU_STATE') ." <span>[".Generals::getTitle('MENU_SUB_ADD')."]</span>");
	$smarty->config_load("state.conf");
	#==================================================================
	#==================================================================
	if ($task):
	...
	$ObjectDao 	= new StateDao();
	$ObjectData = $ObjectDao->getData();
	$Ordering 	= $ObjectDao->getOrdering();
	$Countries 	= $ObjectDao->getCountries();
	$Cities	 	= $ObjectDao->getCities($ObjectData['country'] ? $ObjectData['country'] : $Countries[0]['id']);
