...
class controler {
    ...
    function getDetailService() {
        ...
        $return['perpax_cost'] = number_format($c_price);
        $return['perpax_sale'] = number_format($price);
        ...
        if (is_array($journeies)) foreach ($journeies as $key => $journey):
            if (is_array($journeies)) foreach ($journey['services'] as $index => $service):
                $total  = array('perpax'=>$total['perpax']+$service['perpax']+$service['perpax']*$service['markup']/100, 'single'=>$total['single']+$service['single']+$service['single']*$service['markup']/100);
                $markup = array('perpax'=>$markup['perpax']+(($service['perpax']*$service['markup'])/100), 'single'=>$markup['single']+(($service['single']*$service['markup'])/100));
