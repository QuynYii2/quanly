...
class controler {
    ...
    function status() {
        ...
        if ($vform['published'] <= 3 && $vform['published'] >= 1):
	...
	function save($task, $function = null) {
        ...
        if (empty($only)):
            ...
            if ($vform['published'] > 3 || $vform['published'] < 1) unset($data['published']);
    ...
    function getDetailService() {
        ...
        $result['cost']     = $result['cost'] + $result['cost']*$result['margin']/100;
        $result['c_extra']  = ($genre == 'hotel' && $person['maxpax']%$result['perpax']) ? $result['extra'] : 0;
        $result['extra']    = ($genre == 'hotel' && $person['maxpax']%$result['perpax']) ? $result['extra'] + $result['extra']*$result['margin']/100 : 0;
        $result['c_single'] = $result['single'];
        $result['single']   = $result['single'] + $result['single']*$result['margin']/100;
        #endif;
        ...
        $return['prices_cost'] = number_format($result['c_cost']*$convert, 2);
        $return['prices_sale'] = number_format($result['cost']*$convert, 2);
        $return['perpax_cost'] = number_format($c_price, 2);
        $return['perpax_sale'] = number_format($price, 2);
        $return['single_cost'] = (!empty($c_single) ? $c_single : '');
        $return['single_sale'] = (!empty($single) ? $single : '');
        $return['extra_cost']  = (!empty($c_extra) ? $c_extra : '');
        $return['extra_sale']  = (!empty($extra) ? $extra : '');
        $return['detail_code'] = $this->_db->getSubServiceCode($genre, $subid);
        #===================================================================================
        #===================================================================================
        $object = array();
        $object['id']           = (int)$js_id;
        $object['profile']      = $parent;
        $object['detail']       = $subid;
        $object['markup']       = $this->_db->getSubServiceMarkup($genre, $subid);;
        $object['price']        = (float)$return['prices_cost'];
        $object['single']       = (float)$return['single_cost'];
        $object['perpax']       = (float)$return['perpax_cost'];
        $object['extra']        = (float)$return['extra_cost'];
        $object['detailname']   = $this->_db->getSubServiceName($genre, $subid);
        $object['detailcode']   = $return['detail_code'];
        $object['p_name']       = $this->_db->getServiceName($parent);
        $object['d_name']       = $this->_db->getSubServiceName($genre, $subid);
        $this->_db->update_db('tbl_journey_service', $object, ' id = ? ', array((int)$object['id']));
        #===================================================================================
        #===================================================================================
        $journeies  = $this->_db->getJourneies(Generals::getVar('qid'));
        if (is_array($journeies)) foreach ($journeies as $i => $journey):
            if (is_array($journey['services'])) foreach ($journey['services'] as $j => $service):
                $journeies[$i]['services'][$j]['profiles'] = $this->_db->getServiceData($service['profile']);
            endforeach;
        endforeach;

        $totals = $profit = 0;
        $total  = $markup = array();
        $detail = $this->_db->getData(Generals::getVar('qid'));
        if (is_array($journeies)) foreach ($journeies as $key => $journey):
            if (is_array($journeies)) foreach ($journey['services'] as $index => $service):
                $total  = array('perpax'=>$total['perpax']+$service['perpax']+$service['perpax']*$service['markup']/100, 'single'=>$total['single']+$service['single']+$service['single']*$service['markup']/100, 'extra'=>$total['extra']+$service['extra']+$service['extra']*$service['markup']/100);
                $markup = array('perpax'=>$markup['perpax']+(($service['perpax']*$service['markup'])/100), 'single'=>$markup['single']+(($service['single']*$service['markup'])/100), 'extra'=>$markup['extra']+(($service['extra']*$service['markup'])/100));
                if ($service['service'] == 'hotel' && ($service['sing_num'] || $service['doub_num'] || $service['xbed_num'])):
                    $totals = $totals + ($service['perpax']+$service['perpax']*$service['markup']/100)*2*$service['doub_num'];
                    $totals = $totals + ($service['single']+$service['single']*$service['markup']/100)*$service['sing_num'];
                    $totals = $totals + ($service['extra']+$service['extra']*$service['markup']/100)*$service['xbed_num'];
        ...
        $return['markup_1'] = number_format($markup['perpax'], 2, '.', ',');
        $return['markup_2'] = number_format($markup['single'], 2, '.', ',');
        $return['markup_3'] = number_format($markup['extra'], 2, '.', ',');
        $return['totals_1'] = number_format($total['perpax'], 2, '.', ',');
        $return['totals_2'] = number_format($total['single'], 2, '.', ',');
        $return['totals_3'] = number_format($total['extra'], 2, '.', ',');
        $return['discount'] = !empty($detail['disgenre']) ? number_format($detail['discount'], 2, '.', ',') : number_format($totals*$detail['discount']/100, 2, '.', ',');
        $return['dis_with'] = number_format($_total, 2, '.', ',');

        $this->_db->update_db('tbl_quotation', array('total' => $_total, 'profit' => $profit), ' id = ? ', array((int)Generals::getVar('qid')));
        #===================================================================================
        #===================================================================================
