<?php
    ...
    $smarty->config_load("exptrans.conf");
	#==================================================================
	#==================================================================
	...
	if ($cid[0]) $smarty->assign("TitlePage", Generals::getTitle('MENU_EXPTRANS') ." <span>[".Generals::getTitle('MENU_SUB_EDIT')."]</span>");
	else $smarty->assign("TitlePage", Generals::getTitle('MENU_EXPTRANS') ." <span>[".Generals::getTitle('MENU_SUB_ADD')."]</span>");
	#==================================================================
	#==================================================================
	if ($task):
	...
	$ObjectDao 	= new ExpTransDao();
	$ObjectData = $ObjectDao->getData();
	$Ordering 	= $ObjectDao->getOrdering();
    $Quotations = $ObjectDao->getQuotations();
    $Profiles   = $ObjectDao->getProfileService($ObjectData['quotation'], array('vehicle', 'boat'));

    if (!is_array($ObjectData['profile'])) $ObjectData['profile'] = explode(',', $ObjectData['profile']);
