...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=hotel&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=hotel&view=form');
		return false;
	}
    ...
    function save($task) {
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."hotel")) {
				$link= 'index.php?option=hotel&view=form&cid[]='. $vform['id'];
            ...
            JFile::copy(IMG_PATH.DS."hotel".DS.Generals::getVar('hiddensocial'), IMG_PATH.DS."hotel".DS.$vform['social']);
		}
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_hotel');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_hotel', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_hotel', $data);
			endif;
            ...
            if ($task == "save" || $task == "copy"):
	        	$link = 'index.php?option=hotel&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=hotel&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_hotel_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_hotel_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
    ...
    function export1() {
        ...
        $sheet->setCellValue('D1', strtoupper(Generals::getTitle('MENU_HOTEL')).' - COSTING')->mergeCells('D1:I'.($next-2));
        ...
        $sheet->setCellValue('G'.$next, Generals::getTitle('S_SINGLE'));
        $sheet->setCellValue('H'.$next, Generals::getTitle('S_COST'));
        $sheet->setCellValue('I'.$next, Generals::getTitle('S_EXTRA'));
        ...
        if (is_array($result)) foreach ($result as $items):
            ...
            $sheet->setCellValue('G'.$next, $items['single'])->getStyle()->getNumberFormat()->setFormatCode('#,##0.00');
            $sheet->setCellValue('H'.$next, $items['cost'])->getStyle()->getNumberFormat()->setFormatCode('#,##0.00');
            $sheet->setCellValue('I'.$next, $items['extra'])->getStyle()->getNumberFormat()->setFormatCode('#,##0.00');
            $sheet->getStyle('G'.$next.':I'.$next)->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
            $next++;
        endforeach;
        ...
        $sheet->setTitle(Generals::getTitle('MENU_HOTEL').' - Costing');
        $objPHPExcel->setActiveSheetIndex(0);

        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="hotel_cost_'.date('Ymd').'.xlsx"');
    ...
    function export2() {
        ...
        $sheet->setCellValue('D1', strtoupper(Generals::getTitle('MENU_HOTEL')).' - SELLING')->mergeCells('D1:I'.($next-2));
        $sheet->getStyle('D1:I'.($next-2))->applyFromArray(array('font' => array('size' => 20), 'alignment' => array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER)));
        ...
        $sheet->setCellValue('E'.$next, Generals::getTitle('TITLE'));
        $sheet->setCellValue('F'.$next, Generals::getTitle('SEASON'));
        $sheet->setCellValue('G'.$next, Generals::getTitle('SINGLE'));
        $sheet->setCellValue('H'.$next, Generals::getTitle('COST'));
        $sheet->setCellValue('I'.$next, Generals::getTitle('EXTRA'));
        $sheet->getStyle('A'.$next.':I'.$next)->applyFromArray(array('font' => array('bold' => true)));
        $next++;
        ...
        if (is_array($result)) foreach ($result as $items):
            ...
            $sheet->setCellValue('E'.$next, $items['name']);
            $sheet->setCellValue('F'.$next, $items['se_name']);
            $sheet->setCellValue('G'.$next, $items['single'] + $items['single']*$items['margin']/100)->getStyle()->getNumberFormat()->setFormatCode('#,##0.00');
            $sheet->setCellValue('H'.$next, $items['cost'] + $items['cost']*$items['margin']/100)->getStyle()->getNumberFormat()->setFormatCode('#,##0.00');
            $sheet->setCellValue('I'.$next, $items['extra'] + $items['extra']*$items['margin']/100)->getStyle()->getNumberFormat()->setFormatCode('#,##0.00');
            $sheet->getStyle('G'.$next.':I'.$next)->getNumberFormat()->setFormatCode( PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
            $next++;
        endforeach;
        ...
        $sheet->setTitle(Generals::getTitle('MENU_HOTEL').' - Selling');
        $objPHPExcel->setActiveSheetIndex(0);

        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="hotel_sell_'.date('Ymd').'.xlsx"');
        header('Cache-Control: max-age=0');
        header('Cache-Control: max-age=1');
        header ('Expires: Mon, 26 Jul 1997 05:00:00 GMT'); // Date in the past
        header ('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT'); // always modified
