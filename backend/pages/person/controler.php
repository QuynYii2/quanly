...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=person&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=person&view=form');
		return false;
	}
	...
	function save($task) {
		...
		$vform['id'] = (int)$cid[0];
        $vform["minpax"]  = $vform["divpax"];
        $vform["maxpax"]  = $vform["divpax"];
        $vform["mindate"] = $vform["mindate"] ? $vform["mindate"] : 1;
        $vform["maxdate"] = $vform["maxdate"] ? $vform["maxdate"] : 1;
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."person")) {
				$link= 'index.php?option=person&view=form&cid[]='. $vform['id'];
		...
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_person');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_person', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_person', $data);
			endif;
	        ...
	        if ($task == "save"):
	        	$link = 'index.php?option=person&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=person&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_person_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_person_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
