...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=spoken&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=spoken&view=form');
		return false;
	}
	...
	function save($task) {
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."spoken")) {
				$link= 'index.php?option=spoken&view=form&cid[]='. $vform['id'];
		...
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_spoken');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_spoken', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_spoken', $data);
			endif;
	        ...
	        if ($task == "save"):
	        	$link = 'index.php?option=spoken&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=spoken&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_spoken_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_spoken_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
