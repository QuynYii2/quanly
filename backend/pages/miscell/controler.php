...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=miscell&view=list');
	}
	...
	function create() {
        ...
        header('location: index.php?option=miscell&view=form');
		return false;
	}
    ...
    function save($task) {
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."miscell")) {
				$link= 'index.php?option=miscell&view=form&cid[]='. $vform['id'];
            ...
            JFile::copy(IMG_PATH.DS."miscell".DS.Generals::getVar('hiddensocial'), IMG_PATH.DS."miscell".DS.$vform['social']);
		}
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_miscell');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_miscell', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_miscell', $data);
			endif;
            ...
            if ($task == "save" || $task == "copy"):
	        	$link = 'index.php?option=miscell&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=miscell&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_miscell_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_miscell_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
