...
class controler {
	...
	function ordering() {
		...
		Generals::redirect('index.php?option=season&view=list');
	}
	...
	function create() {
		...
		header('location: index.php?option=season&view=form');
		return false;
	}
	...
	function save($task) {
		...
		$vform['id'] = (int)$cid[0];
        $vform["startdate"] = $vform["startdate"] ? date('Y-m-d H:i:s', strtotime($vform["startdate"])) : null;
        $vform["enddate"] 	= $vform["enddate"] ? date('Y-m-d H:i:s', strtotime($vform["enddate"])) : null;
		...
		if($file_social['name'] != '')  {
			if (!Generals::addFile('social', 'normal', IMG_PATH.DS."season")) {
				$link= 'index.php?option=season&view=form&cid[]='. $vform['id'];
		...
		#=========================================================================================
		#=========================================================================================
		$data = $this->_db->getDataMapTable($vform, 'tbl_season');

		$this->_db->beginTrans();
		try {
			if ($data['id']):
				$this->_db->update_db('tbl_season', $data, ' id = ? ', array($data['id']));
			else:
				$this->_db->insert_db('tbl_season', $data);
			endif;
	        ...
	        if ($task == "save"):
	        	$link = 'index.php?option=season&view=list';
	        elseif ($task == "apply"):
	        	$link = 'index.php?option=season&view=form&cid[]='. $vform['id'] .'';
	        endif;
		} catch (Exception $ex) {
        ...
        try {
	        if (is_array($vform["name"])) foreach ($vform["name"] as $lang => $name):
	        	...
	        	if ($data['id']):
					$this->_db->update_db('tbl_season_lang', $data, ' id = ? ', array($data['id']));
				else:
					$this->_db->insert_db('tbl_season_lang', $data);
				endif;
        	endforeach;
        	$this->_db->commitTrans();
        } catch (Exception $ex) {
