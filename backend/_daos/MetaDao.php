<?php
 if (!defined("META_DAO_INC")) {
	define("META_DAO_INC",1);
	
	class MetaDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getLoginData($metaname, $password) {
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_meta AS a ';
			$query.= ' LEFT JOIN tbl_meta_lang AS fl ON a.id = fl.meta';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = fl.language ';
			$query.= ' WHERE TRIM(LOWER(a.metaname)) = ? AND a.password = ? AND a.published = 1 ';
			$this->prepare($query);
			
			$result = $this->exec(array(trim(strtolower($metaname)), md5($password)));
			
			return isset($result[0]) ? $result[0] : array();
		}
		
		function getDataByEmail($email, $void) {
			$query = 'SELECT a.* FROM tbl_meta AS a WHERE TRIM(LOWER(a.email)) = ? AND a.id <> ? ';
			$this->prepare($query);
		...
		function getDataByMeta($metaname, $void) {
			$query = 'SELECT a.* FROM tbl_meta AS a WHERE TRIM(LOWER(a.metaname)) = ? AND a.id <> ? ';
			$this->prepare($query);
			$result = $this->exec(array(trim(strtolower($metaname)), (int)$void));
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_meta_lang WHERE meta = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_meta AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_meta AS a ';
			$query.= ' INNER JOIN tbl_meta_lang AS b ON a.id = b.meta WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_meta SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_meta SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_meta', " id = ? ", array($oid));
			$this->delete_db('tbl_meta_lang', " meta = ? ", array($oid));
		}
	}// end class		
 }
?>