<?php
 if (!defined("FAQ_DAO_INC")) {
	define("FAQ_DAO_INC",1);
	
	class FaqDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_faq_lang WHERE faq = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_faq AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_faq AS a ';
			$query.= ' INNER JOIN tbl_faq_lang AS b ON a.id = b.faq WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.shorttext, fl.tags, fl.meta FROM tbl_faq AS a ';
			$query.= ' LEFT JOIN tbl_faq_lang AS fl ON a.id = fl.faq';
		...
		function feature($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_faq SET feature = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_faq SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_faq SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_faq', " id = ? ", array($oid));
			$this->delete_db('tbl_faq_lang', " faq = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."faq".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."faq".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>