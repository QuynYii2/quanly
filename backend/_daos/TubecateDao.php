<?php
 if (!defined("TUBECATE_DAO_INC")) {

	define("TUBECATE_DAO_INC",1);
	
	class TubecateDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getLevel($id) {
			$query = 'SELECT a.level FROM tbl_tubecate AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getTubecateLang($language = null, $tubecate = null) {
			$tubecate = $tubecate ? $tubecate : $this->_id;
			$query = ' SELECT * FROM tbl_tubecate_lang WHERE tubecate = ? AND language = ? ';
			$this->prepare($query);
			$result = $this->exec(array($tubecate, $language));
			
			return $result[0];
		}
		
		function getData() {
			$query = 'SELECT a.* FROM tbl_tubecate AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			...
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_tubecate AS a ';
			$query.= ' INNER JOIN tbl_tubecate_lang AS b ON a.id = b.tubecate WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getCategoryById($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.tags, fl.meta FROM tbl_tubecate AS a ';
			$query.= ' LEFT JOIN tbl_tubecate_lang AS fl ON a.id = fl.tubecate';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_tubecate SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_tubecate SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getCategoryById($oid);
			$this->delete_db('tbl_tubecate', " id = ? ", array($oid));
			$this->delete_db('tbl_tubecate_lang', " tubecate = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."tubecate".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."tubecate".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>