<?php
 if (!defined("PARTNER_DAO_INC")) {
	define("PARTNER_DAO_INC",1);
	
	class PartnerDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getCountries() {
			$query = 'SELECT a.id AS value, a.name AS text FROM tbl_country AS a WHERE a.published = 1 ORDER BY name ASC ';
			$this->prepare($query);
		...
		function getCities($country) {
			$query = 'SELECT a.id AS value, a.name AS text FROM tbl_city AS a WHERE a.published = 1 AND a.country = '.(int)$country.' ORDER BY name ASC ';
			$this->prepare($query);
			$result = $this->exec(array());
		...
		function getStates($city) {
			$query = 'SELECT a.id AS value, a.name AS text FROM tbl_state AS a WHERE a.published = 1 AND a.city = '.(int)$city.' ORDER BY name ASC ';
			$this->prepare($query);
			$result = $this->exec(array());
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_partner_lang WHERE partner = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_partner AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_partner AS a ';
			$query.= ' INNER JOIN tbl_partner_lang AS b ON a.id = b.partner WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.shorttext, fl.tags, fl.meta FROM tbl_partner AS a ';
			$query.= ' LEFT JOIN tbl_partner_lang AS fl ON a.id = fl.partner';
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.*, cl.name AS catename, fl.name, fl.alias, fl.introtext, fl.shorttext, fl.tags, fl.meta FROM tbl_partner AS a ';
			$query.= ' LEFT JOIN tbl_partner_lang AS fl ON a.id = fl.partner';
			$query.= ' LEFT JOIN tbl_catpartner_lang AS cl ON cl.catpartner = a.catpartner';
		...
		function feature($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_partner SET feature = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_partner SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_partner SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_partner', " id = ? ", array($oid));
			$this->delete_db('tbl_partner_lang', " partner = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."partner".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."partner".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>