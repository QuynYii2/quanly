<?php
 if (!defined("RECRUITMENT_DAO_INC")) {
	define("RECRUITMENT_DAO_INC",1);
	
	class RecruitmentDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_recruitment_lang WHERE recruitment = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_recruitment AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_recruitment AS a ';
			$query.= ' INNER JOIN tbl_recruitment_lang AS b ON a.id = b.recruitment WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.shorttext, fl.tags, fl.meta FROM tbl_recruitment AS a ';
			$query.= ' LEFT JOIN tbl_recruitment_lang AS fl ON a.id = fl.recruitment';
		...
		function feature($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_recruitment SET feature = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_recruitment SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_recruitment SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_recruitment', " id = ? ", array($oid));
			$this->delete_db('tbl_recruitment_lang', " recruitment = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."recruitment".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."recruitment".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>