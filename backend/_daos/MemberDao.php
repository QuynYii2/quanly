 ...
 if (!defined("MEMBER_DAO_INC")) {
	...
	class MemberDao extends DbConnect /*DbMySQLConnect*/  {
		...
		function getDataByEmail($email, $void) {
			$query = 'SELECT a.* FROM tbl_member AS a WHERE TRIM(LOWER(a.email)) = ? AND a.id <> ? ';
			$this->prepare($query);
		...
		function getDataByUser($username, $void) {
			$query = 'SELECT a.* FROM tbl_member AS a WHERE TRIM(LOWER(a.username)) = ? AND a.id <> ? ';
			$this->prepare($query);
		...
		function getData($void = null) {
			...
			$query 	= 'SELECT a.* FROM tbl_member AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, a.name AS text FROM tbl_member AS a ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.* FROM tbl_member AS a '.$where;
		...
		function frontend($val, $keys) {
			if ($val):
				$this->prepare(' UPDATE tbl_member SET frontend = 0 ');
				$this->exec(array());
			endif;
			
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_member SET frontend = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_member SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_member SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getData($oid);
			$this->delete_db('tbl_member', " id = ? ", array($oid));
