<?php
 if (!defined("NEWSLETTER_DAO_INC")) {
	define("NEWSLETTER_DAO_INC",1);
	
	class NewsletterDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		public function getNewsletters() {
			$query = " SELECT a.* FROM tbl_newsletter AS a WHERE a.published = 1 ORDER BY a.ordering ASC ";
			$this->prepare($query);
		...
		function getDataByCode($code, $void) {
			$query = 'SELECT a.* FROM tbl_newsletter AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
			$this->prepare($query);
		...
		function getData($void = null) {
			...
			$query 	= 'SELECT a.* FROM tbl_newsletter AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, a.name AS text FROM tbl_newsletter AS a ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array());
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.* FROM tbl_newsletter AS a '.$where;
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_newsletter SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_newsletter SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getData($oid);
			$this->delete_db('tbl_newsletter', " id = ? ", array($oid));
