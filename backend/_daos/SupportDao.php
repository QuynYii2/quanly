 ...
 if (!defined("SUPPORT_DAO_INC")) {
	...
	class SupportDao extends DbConnect /*DbMySQLConnect*/  {
		...
		function getDataByUser($username, $void) {
			$query = 'SELECT a.* FROM tbl_support AS a WHERE TRIM(LOWER(a.username)) = ? AND a.id <> ? ';
			$this->prepare($query);
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_support_lang WHERE support = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_support AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_support AS a ';
			$query.= ' INNER JOIN tbl_support_lang AS b ON a.id = b.support WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_support AS a ';
			$query.= ' LEFT JOIN tbl_support_lang AS fl ON a.id = fl.support';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_support SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_support SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_support', " id = ? ", array($oid));
			$this->delete_db('tbl_support_lang', " support = ? ", array($oid));
		}
	}// end class		
 }
?>