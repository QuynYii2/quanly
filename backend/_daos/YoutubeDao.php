<?php
 if (!defined("YOUTUBE_DAO_INC")) {
	define("YOUTUBE_DAO_INC",1);
	
	class YoutubeDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getCateInId($ids = 0, $language = 'vn'){
		 	$query = ' SELECT a.*, b.name, b.alias, b.introtext FROM tbl_tubecate AS a ';
			$query.= ' LEFT JOIN tbl_tubecate_lang AS b ON a.id = b.tubecate';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = b.language ';
			$query.= ' WHERE a.id IN ('.$ids.') AND b.language = ? ';
			$this->prepare($query);
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_youtube_lang WHERE youtube = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_youtube AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_youtube AS a ';
			$query.= ' INNER JOIN tbl_youtube_lang AS b ON a.id = b.youtube WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.shorttext, fl.tags, fl.meta FROM tbl_youtube AS a ';
			$query.= ' LEFT JOIN tbl_youtube_lang AS fl ON a.id = fl.youtube';
		...
		function feature($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_youtube SET feature = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_youtube SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_youtube SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_youtube', " id = ? ", array($oid));
			$this->delete_db('tbl_youtube_lang', " youtube = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."youtube".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."youtube".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>