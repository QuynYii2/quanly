<?php
 if (!defined("REPLY_DAO_INC")) {
	define("REPLY_DAO_INC",1);
	
	class ReplyDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getInquires() {
			$query 	= 'SELECT a.* FROM tbl_inquire AS a ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getData($void = null) {
			...
			$query 	= 'SELECT a.* FROM tbl_reply AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, a.id AS text FROM tbl_reply AS a ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array());
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.*, b.name FROM tbl_reply AS a ';
			$query.= ' INNER JOIN tbl_inquire AS b ON a.inquire = b.id '.$where;
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_reply SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_reply SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getData($oid);
			$this->delete_db('tbl_reply', " id = ? ", array($oid));
