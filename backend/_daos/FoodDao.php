	...
	class FoodDao extends DbConnect /*DbMySQLConnect*/  {
		...
		function getState() {
			return array(
                            ...
                            'filter_service' => Generals::getState('filter.service')
						);
		}
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_food AS a ';
			$query.= ' LEFT JOIN tbl_food_lang AS fl ON a.id = fl.food';
        ...
        function getServices($id = null){
            ...
            $query.= ' AND d.language = ? AND a.genre = "food" ORDER BY b.name ';
