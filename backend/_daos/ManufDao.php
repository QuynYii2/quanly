<?php
 if (!defined("MANUF_DAO_INC")) {

	define("MANUF_DAO_INC",1);
	
	class ManufDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getLevel($id) {
			$query = 'SELECT a.level FROM tbl_manuf AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getManufLang($language = null, $manuf = null) {
			$manuf = $manuf ? $manuf : $this->_id;
			$query = ' SELECT * FROM tbl_manuf_lang WHERE manuf = ? AND language = ? ';
			$this->prepare($query);
			$result = $this->exec(array($manuf, $language));
			
			return $result[0];
		}
		
		function getData() {
			$query = 'SELECT a.* FROM tbl_manuf AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			...
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_manuf AS a ';
			$query.= ' INNER JOIN tbl_manuf_lang AS b ON a.id = b.manuf WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getCategoryById($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.tags, fl.meta FROM tbl_manuf AS a ';
			$query.= ' LEFT JOIN tbl_manuf_lang AS fl ON a.id = fl.manuf';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_manuf SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_manuf SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getCategoryById($oid);
			$this->delete_db('tbl_manuf', " id = ? ", array($oid));
			$this->delete_db('tbl_manuf_lang', " manuf = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."manuf".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."manuf".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>