<?php
 if (!defined("CLASSIFIED_DAO_INC")) {
	define("CLASSIFIED_DAO_INC",1);
	
	class ClassifiedDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getCities($country = null) {
			$query = ' SELECT a.* FROM tbl_city AS a WHERE a.published = 1 ';
			if ($country) $query.= ' AND a.country = '.(int)$country;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_classified_lang WHERE classified = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_classified AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_classified AS a ';
			$query.= ' INNER JOIN tbl_classified_lang AS b ON a.id = b.classified WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.shorttext FROM tbl_classified AS a ';
			$query.= ' LEFT JOIN tbl_classified_lang AS fl ON a.id = fl.classified';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_classified SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_classified SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$this->delete_db('tbl_classified', " id = ? ", array($oid));
			$this->delete_db('tbl_classified_lang', " classified = ? ", array($oid));
		}
	}// end class		
 }
?>