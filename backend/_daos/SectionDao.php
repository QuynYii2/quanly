<?php
 if (!defined("SECTION_DAO_INC")) {

	define("SECTION_DAO_INC",1);
	
	class SectionDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getPosition() {
			return array(1 => Generals::getTitle('POSITION_1'), 2 => Generals::getTitle('POSITION_2'), 3 => Generals::getTitle('POSITION_3'), 4 => Generals::getTitle('POSITION_4'), 5 => Generals::getTitle('POSITION_5'));
		}
		
		function getParams() {
			return array(1 => Generals::getTitle('SECTION_PARAM_1'), 2 => Generals::getTitle('SECTION_PARAM_2'));
		}
		
		function getLevel($id) {
			$query = 'SELECT a.level FROM tbl_section AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getSectionLang($language = null, $section = null) {
			$section = $section ? $section : $this->_id;
			$query = ' SELECT * FROM tbl_section_lang WHERE section = ? AND language = ? ';
			$this->prepare($query);
			$result = $this->exec(array($section, $language));
			
			return $result[0];
		}
		
		function getData() {
			$query = 'SELECT a.* FROM tbl_section AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			...
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_section AS a ';
			$query.= ' INNER JOIN tbl_section_lang AS b ON a.id = b.section WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getCategoryById($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.tags, fl.meta FROM tbl_section AS a ';
		...
		function getCategoryByParentId($parent = 0){
			if (Generals::getVar('option') == 'section'):
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_section SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_section SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getCategoryById($oid);
			$this->delete_db('tbl_section', " id = ? ", array($oid));
			$this->delete_db('tbl_section_lang', " section = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."section".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."section".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>