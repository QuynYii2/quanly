<?php
 if (!defined("PIMAGE_DAO_INC")) {
	define("PIMAGE_DAO_INC",1);
	
	class PimageDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getProductList() {
			$query = ' SELECT a.id, fl.name FROM tbl_product AS a ';
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_product_image_lang WHERE image = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_product_image AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_product_image AS a ';
			$query.= ' INNER JOIN tbl_product_image_lang AS b ON a.id = b.image WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array(Generals::getSession('langcode')));
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, pr.name AS pro_name, fl.name, fl.alias, fl.introtext, fl.shorttext, fl.tags, fl.meta FROM tbl_product_image AS a ';
		 	$query.= ' LEFT JOIN tbl_product_lang AS pr ON a.product = pr.product ';
			$query.= ' LEFT JOIN tbl_product_image_lang AS fl ON a.id = fl.image';
			$query.= ' LEFT JOIN tbl_language AS l ON l.code = fl.language ';
			$query.= ' WHERE fl.language = ? AND pr.language = ? AND a.id = '.(int)$id;
			$this->prepare($query);
			$result = $this->exec(array(Generals::getSession('langcode'), Generals::getSession('langcode')));
		...
		function feature($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_product_image SET feature = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_product_image SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_product_image SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_product_image', " id = ? ", array($oid));
			$this->delete_db('tbl_product_image_lang', " image = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."pimage".DS.$result['social']);
			JFile::delete(IMG_PATH.DS."product".DS."multiple".DS.$result['image']);
		}
	}// end class		
 }
?>