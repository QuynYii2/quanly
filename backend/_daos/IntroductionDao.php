<?php
 if (!defined("INTRODUCTION_DAO_INC")) {
	define("INTRODUCTION_DAO_INC",1);
	
	class IntroductionDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_introduction_lang WHERE introduction = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_introduction AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_introduction AS a ';
			$query.= ' INNER JOIN tbl_introduction_lang AS b ON a.id = b.introduction WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.shorttext, fl.tags, fl.meta FROM tbl_introduction AS a ';
			$query.= ' LEFT JOIN tbl_introduction_lang AS fl ON a.id = fl.introduction';
		...
		function feature($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_introduction SET feature = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_introduction SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_introduction SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_introduction', " id = ? ", array($oid));
			$this->delete_db('tbl_introduction_lang', " introduction = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."introduction".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."introduction".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>