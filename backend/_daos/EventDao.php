<?php
 if (!defined("EVENT_DAO_INC")) {
	define("EVENT_DAO_INC",1);
	
	class EventDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getImages() {
		 	$query = ' SELECT * FROM tbl_event_image WHERE event = ? ';
			$this->prepare($query);
		...
		function getVideos() {
		 	$query = ' SELECT * FROM tbl_event_video WHERE event = ? ';
			$this->prepare($query);
			$result = $this->exec(array($this->_id));
		...
		function getMembers() {
		 	$query = ' SELECT * FROM tbl_event_member WHERE event = ? ';
			$this->prepare($query);
			$result = $this->exec(array($this->_id));
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_event_lang WHERE event = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_event AS a WHERE a.id = ? ';
			$this->prepare($query);
			$result = $this->exec(array($this->_id));
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_event AS a ';
			$query.= ' INNER JOIN tbl_event_lang AS b ON a.id = b.event WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.shorttext, fl.tags, fl.meta FROM tbl_event AS a ';
			$query.= ' LEFT JOIN tbl_event_lang AS fl ON a.id = fl.event';
		...
		function feature($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_event SET feature = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_event SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_event SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_event', " id = ? ", array($oid));
			$this->delete_db('tbl_event_lang', " event = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."event".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."event".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>