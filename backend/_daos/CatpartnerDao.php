<?php
 if (!defined("CATPARTNER_DAO_INC")) {

	define("CATPARTNER_DAO_INC",1);
	
	class CatpartnerDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getLevel($id) {
			$query = 'SELECT a.level FROM tbl_catpartner AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getCatpartnerLang($language = null, $catpartner = null) {
			$catpartner = $catpartner ? $catpartner : $this->_id;
			$query = ' SELECT * FROM tbl_catpartner_lang WHERE catpartner = ? AND language = ? ';
			$this->prepare($query);
			$result = $this->exec(array($catpartner, $language));
			
			return $result[0];
		}
		
		function getData() {
			$query = 'SELECT a.* FROM tbl_catpartner AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			...
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_catpartner AS a ';
			$query.= ' INNER JOIN tbl_catpartner_lang AS b ON a.id = b.catpartner WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getCatpartnerById($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.tags, fl.meta FROM tbl_catpartner AS a ';
			$query.= ' LEFT JOIN tbl_catpartner_lang AS fl ON a.id = fl.catpartner';
		...
		function getCatpartnerByParentId($parent = 0){
		...
		function getCategories($parentid = 0, $_level = null, $insert_text = null) {
			$_result = $this->getCatpartnerByParentId((int)$parentid);
			if (!empty($_result)) :
		...
		function getDataList($parentid = 0, $_level = null, $insert_text = null) {
			$this->dhtml = null;
			if ($parentid) $result = $this->getCatpartnerById((int)$parentid);
			if (!empty($result)) :
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_catpartner SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_catpartner SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getCatpartnerById($oid);
			$this->delete_db('tbl_catpartner', " id = ? ", array($oid));
			$this->delete_db('tbl_catpartner_lang', " catpartner = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."catpartner".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."catpartner".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>