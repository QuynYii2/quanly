<?php
 if (!defined("CLASSIFY_DAO_INC")) {

	define("CLASSIFY_DAO_INC",1);
	
	class ClassifyDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getLevel($id) {
			$query = 'SELECT a.level FROM tbl_classify AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getClassifyLang($language = null, $classify = null) {
			$classify = $classify ? $classify : $this->_id;
			$query = ' SELECT * FROM tbl_classify_lang WHERE classify = ? AND language = ? ';
			$this->prepare($query);
			$result = $this->exec(array($classify, $language));
			
			return $result[0];
		}
		
		function getData() {
			$query = 'SELECT a.* FROM tbl_classify AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			...
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_classify AS a ';
			$query.= ' INNER JOIN tbl_classify_lang AS b ON a.id = b.classify WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getCategoryById($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.tags, fl.meta FROM tbl_classify AS a ';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_classify SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_classify SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getCategoryById($oid);
			$this->delete_db('tbl_classify', " id = ? ", array($oid));
			$this->delete_db('tbl_classify_lang', " classify = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."classify".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."classify".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>