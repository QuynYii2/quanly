<?php
 if (!defined("COMFORT_DAO_INC")) {
	define("COMFORT_DAO_INC",1);
	
	class ComfortDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_comfort_lang WHERE comfort = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_comfort AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_comfort AS a ';
			$query.= ' INNER JOIN tbl_comfort_lang AS b ON a.id = b.comfort WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext, fl.shorttext FROM tbl_comfort AS a ';
			$query.= ' LEFT JOIN tbl_comfort_lang AS fl ON a.id = fl.comfort';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_comfort SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_comfort SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$this->delete_db('tbl_comfort', " id = ? ", array($oid));
			$this->delete_db('tbl_comfort_lang', " comfort = ? ", array($oid));
		}
	}// end class		
 }
?>