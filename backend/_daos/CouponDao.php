<?php
 if (!defined("COUPON_DAO_INC")) {
	define("COUPON_DAO_INC",1);
	
	class CouponDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_coupon_lang WHERE coupon = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_coupon AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_coupon AS a ';
			$query.= ' INNER JOIN tbl_coupon_lang AS b ON a.id = b.coupon WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_coupon AS a ';
			$query.= ' LEFT JOIN tbl_coupon_lang AS fl ON a.id = fl.coupon';
		...
		function _buildQuery(){
			...
			$where[] = ' fl.language = ? ';
			if (strlen($search)):
				$where[] = ' (LOWER(fl.name) LIKE ? OR LOWER(a.code) LIKE ?) ';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_coupon SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_coupon SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_coupon', " id = ? ", array($oid));
			$this->delete_db('tbl_coupon_lang', " coupon = ? ", array($oid));
		}
	}// end class		
 }
?>