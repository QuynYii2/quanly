<?php
 if (!defined("INQUIRE_DAO_INC")) {
	define("INQUIRE_DAO_INC",1);
	
	class InquireDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getData($void = null) {
			...
			$query 	= 'SELECT a.* FROM tbl_inquire AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, a.name AS text FROM tbl_inquire AS a ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.* FROM tbl_inquire AS a '.$where;
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_inquire SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function feature($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_inquire SET feature = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_inquire SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getData($oid);
			$this->delete_db('tbl_inquire', " id = ? ", array($oid));
