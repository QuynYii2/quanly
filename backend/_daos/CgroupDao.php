<?php
 if (!defined("CGROUP_DAO_INC")) {
	define("CGROUP_DAO_INC",1);
	
	class CgroupDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_cgroup_lang WHERE cgroup = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_cgroup AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_cgroup AS a ';
			$query.= ' INNER JOIN tbl_cgroup_lang AS b ON a.id = b.cgroup WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_cgroup SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_cgroup SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$this->delete_db('tbl_cgroup', " id = ? ", array($oid));
			$this->delete_db('tbl_cgroup_lang', " cgroup = ? ", array($oid));
		}
	}// end class		
 }
?>