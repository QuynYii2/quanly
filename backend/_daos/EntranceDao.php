 ...
 if (!defined("ENTRANCE_DAO_INC")) {
	...
	class EntranceDao extends DbConnect /*DbMySQLConnect*/  {
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_entrance AS a ';
			$query.= ' LEFT JOIN tbl_entrance_lang AS fl ON a.id = fl.entrance';
        ...
        function getServices($id = null){
            ...
            $query.= ' AND d.language = ? AND a.genre = "entrance" ORDER BY b.name ';
