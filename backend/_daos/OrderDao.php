<?php
 if (!defined("ORDER_DAO_INC")) {
	define("ORDER_DAO_INC",1);
	
	class OrderDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		public function getOrderItems() {
			$query = " SELECT a.* FROM tbl_order_item AS a WHERE a.orderid = ".(int)$this->_id." ORDER BY a.id ASC ";
			$this->prepare($query);
		...
		function getData($void = null) {
			...
			$query 	= 'SELECT a.* FROM tbl_order AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, a.name AS text FROM tbl_order AS a ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array());
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.* FROM tbl_order AS a '.$where;
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_order SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_order SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$this->delete_db('tbl_order', " id = ? ", array($oid));
			$this->delete_db('tbl_order_item', " orderid = ? ", array($oid));
		}
	}// end class		
 }
?>