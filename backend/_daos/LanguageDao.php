<?php
 if (!defined("LANGUAGE_DAO_INC")) {
	define("LANGUAGE_DAO_INC",1);
	
	class LanguageDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		public function getLanguages() {
			$query = " SELECT a.* FROM tbl_language AS a WHERE a.published = 1 ORDER BY a.ordering ASC ";
			$this->prepare($query);
		...
		function getDataByCode($code, $void) {
			$query = 'SELECT a.* FROM tbl_language AS a WHERE TRIM(LOWER(a.code)) = ? AND a.id <> ? ';
			$this->prepare($query);
		...
		function getData($void = null) {
			...
			$query 	= 'SELECT a.* FROM tbl_language AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, a.name AS text FROM tbl_language AS a ORDER BY a.ordering ';
			$this->prepare($query);
			$result = $this->exec(array());
		...
		function _buildQuery(){
			...
			$query = ' SELECT a.* FROM tbl_language AS a '.$where;
		...
		function frontend($val, $keys) {
			if ($val):
				$this->prepare(' UPDATE tbl_language SET frontend = 0 ');
				$this->exec(array());
			endif;
			
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_language SET frontend = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_language SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_language SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getData($oid);
			$this->delete_db('tbl_language', " id = ? ", array($oid));
