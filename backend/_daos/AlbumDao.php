<?php
 if (!defined("ALBUM_DAO_INC")) {
	define("ALBUM_DAO_INC",1);
	
	class AlbumDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_album_lang WHERE album = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_album AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_album AS a ';
			$query.= ' INNER JOIN tbl_album_lang AS b ON a.id = b.album WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
			...
			$query.= ' WHERE fl.language = ? AND a.id = '.(int)$id;
			$this->prepare($query);
			$result = $this->exec(array(Generals::getSession('langcode')));
		...
		public function delete($oid) {
			...
			$this->delete_db('tbl_album', " id = ? ", array($oid));
			$this->delete_db('tbl_album_lang', " album = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."album".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."album".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>