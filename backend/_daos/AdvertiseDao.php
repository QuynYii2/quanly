<?php
 if (!defined("ADVERTISE_DAO_INC")) {
	define("ADVERTISE_DAO_INC",1);
	
	class AdvertiseDao extends DbConnect /*DbMySQLConnect*/  {
		var $_id;
		...
		function getDataLang($language = null, $void = null) {
			...
			$query = ' SELECT * FROM tbl_advertise_lang WHERE advertise = ? AND language = ? ';
			$this->prepare($query);
		...
		function getData() {
			$query = 'SELECT a.* FROM tbl_advertise AS a WHERE a.id = ? ';
			$this->prepare($query);
		...
		function getOrdering() {
			$query = ' SELECT a.ordering AS value, b.name AS text FROM tbl_advertise AS a ';
			$query.= ' INNER JOIN tbl_advertise_lang AS b ON a.id = b.advertise WHERE b.language = ? ORDER BY a.ordering ';
			$this->prepare($query);
		...
		function getDataFull($id){
		 	$query = ' SELECT a.*, fl.name, fl.alias, fl.introtext FROM tbl_advertise AS a ';
			$query.= ' LEFT JOIN tbl_advertise_lang AS fl ON a.id = fl.advertise';
		...
		function published($val, $keys) {
			if (is_array($keys)) foreach ($keys as $key):
				$this->prepare(' UPDATE tbl_advertise SET published = '.(int)$val.' WHERE id = '.(int)$key);
				$this->exec(array());
			endforeach;
		}
		
		function ordering($val, $key) {
			$this->prepare(' UPDATE tbl_advertise SET ordering = '.(int)$val.' WHERE id = '.(int)$key);
			$this->exec(array());
		}
		
		public function delete($oid) {
			$result = $this->getDataFull($oid);
			$this->delete_db('tbl_advertise', " id = ? ", array($oid));
			$this->delete_db('tbl_advertise_lang', " advertise = ? ", array($oid));
			
			JFile::delete(IMG_PATH.DS."advertise".DS.$result['image']);
			JFile::delete(IMG_PATH.DS."advertise".DS.'resize'.DS.str_replace('image', 'thumb', $result['image']));
		}
	}// end class		
 }
?>